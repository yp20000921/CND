...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        self.rd = config["model"]["decoder"]['rd']
        self.curriculum_type = config["model"]["decoder"]['curriculum_type']
        ...
        if self.eval_metric not in ["bleu", "chrf", "wer", "rouge","bleu_nat"]:
            raise ValueError(
        ...
        if self.early_stopping_metric in [
            ...
            if self.eval_metric in ["bleu", "chrf", "rouge","bleu_nat"]:
    ...
    def _save_checkpoint(self,epoch) -> None:
        ...
        """
        model_path = "{}/{}_{}.ckpt".format(self.model_dir, self.steps,epoch)
        state = {
            "steps": self.steps,
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) -> None:
        ...
        for epoch_no in range(self.epochs):
            ...
            #self._save_checkpoint(epoch_no)

            train_ratio = (epoch_no+1)/self.epochs
            ...
            for batch in iter(train_iter):
                ...
                recognition_loss, translation_loss,_,_,_ = self._train_batch(
                    batch=batch, update=update
                )
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    self.logger.info(
                        ...
                        "CHRF %.2f\t"
                        "ROUGE %.2f\n\t"
                        "nat bleu-4 %.2f\t(BLEU-1: %.2f,\tBLEU-2: %.2f,\tBLEU-3: %.2f,\tBLEU-4: %.2f)\n\t)",
                        epoch_no + 1,
                        self.steps,
                        valid_duration,
                        -1,
                        ...
                        val_res["valid_scores"]["bleu_nat"],
                        val_res["valid_scores"]["bleu_scores_nat"]["bleu1"],
                        val_res["valid_scores"]["bleu_scores_nat"]["bleu2"],
                        val_res["valid_scores"]["bleu_scores_nat"]["bleu3"],
                        val_res["valid_scores"]["bleu_scores_nat"]["bleu4"],
    ...
    def _train_batch(self,batch: Batch, update: bool = True) -> (Tensor, Tensor):
        ...
        recognition_loss, translation_loss,at,nat,kl = self.model.get_loss_for_batch(
            batch=batch,
            rd=self.rd,
            curriculum_type=self.curriculum_type,
            translation_loss_function=self.translation_loss_function
        ...
        if self.do_recognition:
            self.total_gls_tokens += batch.num_txt_tokens
        if self.do_translation:
        ...
        return normalized_recognition_loss, normalized_translation_loss,at,nat,kl
