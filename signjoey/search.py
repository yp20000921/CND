...
from signjoey.decoders import Decoder, TransformerDecoder_at
...
def transformer_greedy(
    ...
    batch_size = src_mask.size(0)  #32

    # start with BOS-symbol for each sentence in the batch
    ys = encoder_output.new_full([batch_size, 1], bos_index, dtype=torch.long)  #[32,1]  [[2][2][2]...]  从bos开始送入decoder

    # a subsequent mask is intersected with this in decoder forward pass
    trg_mask = src_mask.new_ones([1, 1, 1])  #src_mask  32,1,165   ？？维度[1,1,1] [[[TRUE]]]
    finished = src_mask.new_zeros((batch_size)).byte()  #[32] 全0
    ...
    for _ in range(max_output_length):
        ...
        with torch.no_grad():
            logits, out, _, _ = decoder(  #logits [32,1,2889]
                trg_embed=trg_embed,
                encoder_output=encoder_output,
                encoder_hidden=None,
                src_mask=src_mask,
            ...
            logits = logits[:, -1] #[32,2889]
            _, next_word = torch.max(logits, dim=1)  #[32]
            next_word = next_word.data
            ys = torch.cat([ys, next_word.unsqueeze(-1)], dim=1) #[32,2]\[32,3]... ...

        # check if previous symbol was <eos>
        is_eos = torch.eq(next_word, eos_index)  #[32] 全false
        finished += is_eos
...
def beam_search(
    ...
    for step in range(max_output_length):
        ...
        topk_beam_index = topk_beam_index.long()# ？
        ...
        alive_seq = torch.cat(
            [alive_seq.index_select(0, select_indices), topk_ids.view(-1, 1)], -1  #取行
