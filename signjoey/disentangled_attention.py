...
from signjoey.ops import *
from signjoey.da_utils import build_relative_position



__all__=['DisentangledSelfAttention']
class DisentangledSelfAttention(nn.Module):
    def __init__(self, num_heads,hidden_size,max_relative_positions,pos_att_type):
        super().__init__()
    ...
    def forward(self, hidden_states, attention_mask, return_att=False, query_states=None, relative_pos=None, rel_embeddings=None,attention_mask_layer=None):
        ...
        # if attention_mask_layer is not None:
        #     attention_scores_mask = attention_scores.clone()
        #     _attention_probs = XSoftmax.apply(attention_scores, attention_mask, -1)
        #     _attention_probs_mask = XSoftmax.apply(attention_scores_mask, attention_mask_layer, -1)
        #     _attention_probs = _attention_probs_mask + _attention_probs
        #
        # else:
    ...
    def disentangled_attention_bias(self, query_layer, key_layer, relative_pos, rel_embeddings, scale_factor):
        ...
        if self.share_att_key:
            ...
            if 'c2p' in self.pos_att_type or 'p2p' in self.pos_att_type: #不执行
                pos_key_layer = self.transpose_for_scores(self.pos_key_proj(rel_embeddings), self.num_attention_heads)\
                    .repeat(key_layer.size(0)//self.num_attention_heads, 1, 1) #.split(self.all_head_size, dim=-1)
            if 'p2c' in self.pos_att_type or 'p2p' in self.pos_att_type:
        ...
        if 'p2c' in self.pos_att_type:  #####################  就是这几行
            p2c_att = torch.bmm(key_layer, pos_query_layer.transpose(-1, -2).to(key_layer)*scale)  #矩阵乘法
            ...
            if query_layer.size(-2) != key_layer.size(-2):  #跟这个里面的也没关系
                pos_index_ = pos_index.squeeze(0)
                index = pos_index_.expand(p2c_att.size()[:1] + (pos_index.size(-2), key_layer.size(-2)))
